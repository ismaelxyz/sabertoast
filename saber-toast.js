function createContainer(direction) {
    let containerName = 'saber-toasts-container';
    const container = document.createElement('div');
    container.classList.add(`${containerName}-${direction}`);
    container.classList.add('saber-toasts-containers');
    document.body.append(container);
}


function initSaberToast() {
    ['bottom-right', 'top-right', 'bottom-left', 'top-left']
        .forEach(createContainer);
}
...
class SaberToast {
    ...
    fire(params) {
        ...
        const { title, text, delay, duration, rtl, position } = 
            { title: "", text: "", delay: 200, duration: 2600, rtl: DEFAULT_RTL, position: "bottom-right", ...params };
